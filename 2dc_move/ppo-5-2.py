...
class PPO:
    ...
    def step(self, cap_idx, action_idx,decap_val,):  # 这里action应该是0-4，代表五个方向,decap_val是可选电容值
        str1 = ''
        while 1:
            action_v =  action_space[action_idx] 
            cap_idxv = cap_idx
            actionv1, cap_idxv1 = move(action_v[1], cap_idxv[action_v[0] * 2 - 2])
            actionv2, cap_idxv2 = move(action_v[2], cap_idxv[action_v[0] * 2 - 1])
            cap_idxv[action_v[0] * 2 - 2] = cap_idxv1
            cap_idxv[action_v[0] * 2 - 1] = cap_idxv2
            if len(set(cap_idxv)) == 9:
                action_idx = random.randint(0,naction - 1)
                continue
            else:
                action = action_space[action_idx]   # action = [5,3,3]
                action1, cap_idx1 = move(action[1], cap_idx[action[0] * 2 - 2])
                action2, cap_idx2 = move(action[2], cap_idx[action[0] * 2 - 1])
                cap_idx[action[0] * 2 - 2] = cap_idx1
                cap_idx[action[0] * 2-1] = cap_idx2
                action[1] = action1
                action[2] = action2
